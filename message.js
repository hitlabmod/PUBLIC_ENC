...
`.trim();
			...
			case 'link':
				...
				case 'tagall':
					if (!m.isOwner && !m.isAdmin) throw 'Perintah ini hanya bisa digunakan oleh owner atau admin';
					if (!m.isGroup) throw 'Perintah ini hanya bisa digunakan di dalam grup';
					let participants = m.metadata.participants.map(p => p.id);
					let mentions = participants.map(p => `@${p.split('@')[0]}`).join(' ');
					await m.reply(mentions, { mentions: participants });
					break;
